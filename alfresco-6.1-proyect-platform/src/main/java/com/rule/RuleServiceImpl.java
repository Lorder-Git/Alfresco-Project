                ...
                NodeServicePolicies.OnAddAspectPolicy
{
    ...
    public void init()
    {
        policyComponent.bindAssociationBehaviour(
                ...
                org.alfresco.repo.rule.RuleModel.ASPECT_RULES,
                org.alfresco.repo.rule.RuleModel.ASSOC_RULE_FOLDER,
        ...
        policyComponent.bindClassBehaviour(
                ...
                org.alfresco.repo.rule.RuleModel.TYPE_RULE,
    ...
    private List<Rule> getRulesForNode(NodeRef nodeRef)
    {
        ...
        if (!runtimeNodeService.hasAspect(nodeRef, org.alfresco.repo.rule.RuleModel.ASPECT_RULES) ||
    ...
    public Rule getRule(NodeRef ruleNodeRef)
    {
        ...
        // Set the rule types
        rule.setRuleTypes((List<String>)props.get(org.alfresco.repo.rule.RuleModel.PROP_RULE_TYPE));
        ...
        List<ChildAssociationRef> actions = this.nodeService.getChildAssocs(ruleNodeRef, org.alfresco.repo.rule.RuleModel.ASSOC_ACTION, org.alfresco.repo.rule.RuleModel.ASSOC_ACTION);
        if (actions.size() == 0)
        {
            ...
            try
            {
                ...
                if (ruleNodeRef == null)
                {
                    ...
                    ruleNodeRef = this.nodeService.createNode(
                            ...
                            QName.createQName(org.alfresco.repo.rule.RuleModel.RULE_MODEL_URI, ASSOC_NAME_RULES_PREFIX + GUID.generate()),
                            org.alfresco.repo.rule.RuleModel.TYPE_RULE).getChildRef();
                ...
                this.nodeService.setProperty(ruleNodeRef, org.alfresco.repo.rule.RuleModel.PROP_RULE_TYPE, (Serializable)rule.getRuleTypes());
                this.nodeService.setProperty(ruleNodeRef, org.alfresco.repo.rule.RuleModel.PROP_APPLY_TO_CHILDREN, rule.isAppliedToChildren());
                this.nodeService.setProperty(ruleNodeRef, org.alfresco.repo.rule.RuleModel.PROP_EXECUTE_ASYNC, rule.getExecuteAsynchronously());
                this.nodeService.setProperty(ruleNodeRef, org.alfresco.repo.rule.RuleModel.PROP_DISABLED, rule.getRuleDisabled());
    ...
    private void saveAction(NodeRef ruleNodeRef, Rule rule)
    {
        ...
        {
            actionNodeRef = this.runtimeActionService.createActionNodeRef(action, ruleNodeRef, org.alfresco.repo.rule.RuleModel.ASSOC_ACTION, org.alfresco.repo.rule.RuleModel.ASSOC_ACTION);
        }
                ...
                if(countRules(nodeRef) == 0)
                {
                    ...
                    if (linkedFrom.isEmpty() == false)
                    {
                        for (NodeRef linkedFromNodeRef : linkedFrom)
                        {
                            ...
                            nodeService.removeAspect(linkedFromNodeRef, org.alfresco.repo.rule.RuleModel.ASPECT_RULES);
                        }
                    }
                    
                    this.nodeService.removeAspect(nodeRef, org.alfresco.repo.rule.RuleModel.ASPECT_RULES);
                }
            }
            // Drop the rules from the cache
            nodeRulesCache.remove(nodeRef);
