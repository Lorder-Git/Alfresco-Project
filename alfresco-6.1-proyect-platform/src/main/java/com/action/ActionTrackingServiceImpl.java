...
public class ActionTrackingServiceImpl implements ActionTrackingService
{
    ...
    private org.alfresco.repo.action.RuntimeActionService runtimeActionService;
    ...
    {
        recordActionPending((org.alfresco.repo.action.ActionImpl) action);
    }
    
    public void recordActionPending(Action action, NodeRef actionedUponNodeRef)
    {
        recordActionPending((org.alfresco.repo.action.ActionImpl) action, actionedUponNodeRef);
    }
    
    public void recordActionPending(org.alfresco.repo.action.ActionImpl action)
    {
        recordActionPending(action, null);
    }

    public void recordActionPending(org.alfresco.repo.action.ActionImpl action, NodeRef actionedUponNodeRef)
    {
        // Set the status
    ...
    {
        recordActionComplete((org.alfresco.repo.action.ActionImpl) action);
    }

    private void recordActionComplete(final org.alfresco.repo.action.ActionImpl action)
    {
                          ...
                          return AuthenticationUtil.runAs(new RunAsWork<Action>() 
                             {
                                public Action doWork() throws Exception
                                {
                                   ...
                                   org.alfresco.repo.action.ActionImpl action = (org.alfresco.repo.action.ActionImpl) runtimeActionService
    ...
    {
        recordActionExecuting((org.alfresco.repo.action.ActionImpl) action, null);
    }
    
    public void recordActionExecuting(Action action, NodeRef actionedUponNodeRef)
    {
        recordActionExecuting((org.alfresco.repo.action.ActionImpl) action, actionedUponNodeRef);
    }
    
    private void recordActionExecuting(org.alfresco.repo.action.ActionImpl action, NodeRef actionedUponNodeRef)
    {
        if (logger.isDebugEnabled() == true)
        {
     ...
     */
    private void placeActionInCache(org.alfresco.repo.action.ActionImpl action, NodeRef actionedUponNodeRef)
    {
    ...
    public void recordActionFailure(Action action, final Throwable exception)
    {
        if (logger.isDebugEnabled() == true)
        {
            if (exception instanceof org.alfresco.repo.action.ActionCancelledException)
            {
        ...
        ((org.alfresco.repo.action.ActionImpl) action).setExecutionEndDate(new Date());

        // Record it as Failed or Cancelled, depending on the exception
        if (exception instanceof org.alfresco.repo.action.ActionCancelledException)
        {
            ((org.alfresco.repo.action.ActionImpl) action).setExecutionStatus(ActionStatus.Cancelled);
            ((org.alfresco.repo.action.ActionImpl) action).setExecutionFailureMessage(null);
        }
        else if (exception instanceof ActionServiceTransientException)
        {
            ((org.alfresco.repo.action.ActionImpl) action).setExecutionStatus(ActionStatus.Declined);
            ((org.alfresco.repo.action.ActionImpl) action).setExecutionFailureMessage(exception.getMessage());
        }
        else
        {
            ((org.alfresco.repo.action.ActionImpl) action).setExecutionStatus(ActionStatus.Failed);
            ((org.alfresco.repo.action.ActionImpl) action).setExecutionFailureMessage(exception.getMessage());
        }
    ...
    protected static String generateCacheKey(Action action)
    {
        return action.getActionDefinitionName() + cacheKeyPartSeparator + action.getId()
                + cacheKeyPartSeparator + ((org.alfresco.repo.action.ActionImpl) action).getExecutionInstance();
    }
